    ...
    public function store(Request $request, $courseId, $lessonId)
{
    ...
    public function showAssessmentResult($courseId, $lessonId)
    {
        ...
        return view('student.assessment_result', compact(
            'userAssessment', 'lesson', 'nextLesson', 'assessmentPassed',
            'allLessonsCompleted', 'routeNamePart', 'courseId', 'lessonId',
            'nextLessonRoute', 'currentLessonRoute', 'retakeLessonRoute', 'retakeAssessmentRoute', 'certificateRoute'
        ));
}
    ...
    private function generateCertificate(User $user, $courseId)
    {
        ...
        $existingCertificate = Storage::exists('certificates/' . $user->id . '/' . $course->id . '.pdf');

        if (!$existingCertificate) {
            $certificate = PDF::loadView('certificates.completion', ['user' => $user, 'course' => $course]);
            Storage::put('certificates/' . $user->id . '/' . $course->id . '.pdf', $certificate->output());
        }
    }
